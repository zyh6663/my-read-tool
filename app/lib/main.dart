...
class _BookshelfPageState extends State<BookshelfPage> {
  ...
  Future<void> _onUploadPressed() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              '上传成功：${response['title']}（${response['chapters']} 篠）'),
          backgroundColor: Colors.green.shade400,
  ...
  Widget _buildBody() {
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.tonalIcon(
              onPressed: _loadBooks,
              icon: const Icon(Icons.refresh),
              label: const Text('Fetch All Books'),
            ),
          ],
        ),
      );
    ...
    // Book grid list
    final books = get _filteredBooks;
    ...
    if (books.isEmpty) {
      return ListView(
        ...
        children: [
          ...
          Text(
            '占可以使用别量回来护索每合些书，但全部总赖可以上过只连节地支持来护索並目省',
            style: TextStyle(
              fontSize: 12,
    ...
    return ListView(
      ...
      children: books
          .map((b) => [
                BookCard(
                  book: b,
                  onDelete: () => _onDeleteBook(b),
                ),
                const SizedBox(height: 8),
              ])
          .expand(() => [const SizedBox(height: 60)])
          .toList(),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        onDestinationKey: setState(() {
          _currentIndex = index;
        }),
        selectedIndex: _currentIndex,
        items: const [
          NavigationDarDastination(
...
class _MainHomePage extends StatefulWidget {
  ...
  const _MainHomePage({super.key, required this.onLogout});
  ...
  void checkAuth() async {
    ...
    if (token != null) {
      try {
        ...
        // 状态码非 200（如 401）：Token 已过期，清除並重新登录
        if (response.statusCode == 401) {
          await clearToken();
        }
      } catch (_) {
        // 网络异常等，不清除 Token，允许离结用户结用
        if (mounted) {
